...
Note:
  If no URL is provided, the script will automatically use the URL copied to your clipboard.
...
const run = async () => {
    ...
    let url = args[0];
    let customOutputFile = args[1];

    // If the first argument isn't a URL, treat it as customOutputFile and fetch URL from clipboard
    if (!url || (!url.startsWith('http://') && !url.startsWith('https://'))) {
        if (url && !url.startsWith('http')) {
            customOutputFile = url;
        }
        url = getClipboardUrl();
        if (url) {
    ...
    if (!url || (!url.startsWith('http://') && !url.startsWith('https://'))) {
        console.error("❌ Error: Invalid URL provided or clipboard does not contain a valid URL.");
        process.exit(1);
    }
